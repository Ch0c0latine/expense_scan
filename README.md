...
À installer dans l'environnement Python d'Odoo, avec `pip` pour les paquets
Python et le gestionnaire de paquets du système pour Tesseract et Poppler :
...
| `openpyxl` | export Excel des fiches de frais | pour cet export |
...
1. **Dépenses → Configuration → Paramètres** : le scan est actif d'office ;
   *Tester et précharger le moteur* vérifie l'installation.
2. **Scannez un ticket** depuis la liste des dépenses (bouton *Scan* sur
   téléphone, *Upload* sur ordinateur).
3. **Règles de dépenses** : l'ensemble livré « Bonnes pratiques (barèmes
   URSSAF) » contrôle déjà chaque dépense. Ajustez ses montants, dupliquez-le
   pour un client, ou archivez-le.
4. **Fiche de frais** : sélectionnez des dépenses, bouton *Fiche de frais*.
   Le modèle Excel livré « Fiche de frais (modèle de base) » se télécharge,
   s'adapte et se redépose (*Configuration → Modèles d'export*).
5. **Missions** (facultatif) : cochez *Refacturer les frais sur une
   mission* dans les paramètres.

Une base créée avec les données de démonstration contient en plus un
client fictif, sa mission et ses exigences propres.

---

## Missions, fiches de frais et règles de dépenses

- **Mission et tâche** : « À refacturer : Oui » porte le frais sur la
  commande du projet ; « Non » peut garder la mission, pour son seul suivi.
  Le frais est imputé sur le compte analytique de la mission (créé au
  besoin) : il apparaît sous le bouton **Dépenses** du projet, et dans sa
  rentabilité une fois comptabilisé. Le champ **Tâche** n'apparaît que si
  la mission a des tâches ouvertes ; la première, dans l'ordre du projet,
  est proposée.
- **Fiche de frais** (bouton à la place d'« Imprimer » sur la liste) :
  récapitulatif PDF avec TVA par taux et justificatifs numérotés, export
  Excel sur modèle (*Configuration → Modèles d'export*), une fiche par
  salarié, réunies dans une archive s'il y en a plusieurs.
- **Règles de dépenses** (*Configuration → Règles de dépenses*) : plafonds
  par repas, par jour ou par nuitée, mots et catégories de location à
  surveiller. L'alerte s'affiche en tête de la dépense et se filtre
  (« Hors règles »). Un ensemble vaut pour toutes les dépenses, pour les
  missions de certains clients, ou pour des missions désignées.
- **Équipe et lots** : *Dépenses de mon équipe* pour saisir au nom d'un
  salarié, et *Actions → Remettre en brouillon* sur une sélection.
...
Le module est libre et le restera. S'il vous fait gagner du temps, vous
pouvez [offrir un café à son auteur](https://github.com/sponsors/Ch0c0latine)
— ou, si vous préférez, le coucher sur votre testament.
